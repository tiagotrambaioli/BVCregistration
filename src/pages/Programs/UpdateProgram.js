...
import { Link as RouterDom, useNavigate, useParams } from 'react-router-dom';
...
export default function UpdateProgram() {
  ...
  useEffect(() => {
    ...
    setDeliveryTypes(program?.deliveryTypes);
    setTuitionDomestic(program?.tuition?.domestic);
    setTuitionInternational(program?.tuition?.international);
    setPeriod(program?.tuition?.period);
    setTerms(program?.terms);
  }, [program]);
